...
class Application():
	...
	def drop_inside_text_box(self,event):
		self.data = event.data[1:-1]
		if event.data.endswith('.txt}'):
			with open(self.data,'r') as self.file:
				for i in self.file:
					self.inp.insert(END,f'{i}')
		else:
			messagebox.showerror('Error','File Cannot Be Opened!!!')

	def drop_inside_text_box1(self,event):
		self.file = open('code.dat','rb')
		self.data1 = pickle.load(self.file)
		self.info = event.data[1:-1]
		self.file.close()
		self.out.delete(1.0,END)
		if event.data.endswith('.txt}'):
			self.out.config(state = NORMAL)
			with open(self.info,'r') as self.file:
				for i in self.file.readlines():
					if i.endswith('\n'):
						for a in range(len(i)-1):
							self.out.insert(END,self.data1[i[a]])
						self.out.insert(END,'\n')
					else:
						for j in range(len(i)):
							self.out.insert(END,self.data1[i[j]])
	...
	def ad_set(self):
		...
		Label(self.win_as,text = 'Information',bg = '#3b3838',font = ('Russo One',25),fg = '#cdcdcd').place(relx =0.455,rely = 0)
		...
		if len(self.data) == 0:
			...
			for j in range(len(self.data)):
				...
				self.as_info.insert(END,'  Password: '+self.data[j][1]+'\n\n')
		self.as_info['state'] = 'disabled'
		Label(self.win_as,text = 'Username:',bg = '#3b3838',font = ('Russo One',15),fg = '#cdcdcd',width = 10).place(relx = 0.034,rely = 0.69)
		...
		Label(self.win_as,text = 'Password:',bg = '#3b3838',font = ('Russo One',15),fg = '#cdcdcd').place(relx = 0.04,rely = 0.795)
		...
		Label(self.win_as,text = 'Confirm Password:',bg = '#3b3838',font = ('Russo One',15),fg = '#cdcdcd').place(relx = 0.04,rely = 0.9)
		...
		self.ma = Button(self.win_as,width = 12,height = 1,text = 'MAKE ADMIN',relief = 'groove',font = ('Russo One',15),bg = '#878181',activebackground = '#878181',borderwidth = 0,command = self.make_ad)
		self.ma.place(relx = 0.65,rely =0.75)
		self.ra = Button(self.win_as,width = 12,height = 1,text = 'REMOVE USER',relief = 'groove',font = ('Russo One',15),bg = '#878181',activebackground = '#878181',borderwidth = 0,command = self.remove_ad)
	...
	def Frame2(self):
		...
		Button(self.frame211,text = 'INPUT',relief = 'groove',font = ('Russo One',10),bg = '#878181',activebackground = '#878181',borderwidth = 0).pack(side = 'top',fill = 'both',expand = 1,padx = 300)
		...
		self.inp.drop_target_register(DND_FILES)
		self.inp.dnd_bind('<<Drop>>',self.drop_inside_text_box)
		self.frame212 = Frame(self.frame21,bg = '#3b3838')
		self.frame212.pack(fill = 'both',expand = 1)
		Button(self.frame212,text = 'OUTPUT',relief = 'groove',font = ('Russo One',10),bg = '#878181',activebackground = '#878181',borderwidth = 0).pack(side = 'top',expand = 1,fill = 'both',padx = 275)
		...
		self.out.drop_target_register(DND_FILES)
		self.out.dnd_bind('<<Drop>>',self.drop_inside_text_box1)
		self.frame22 = Frame(self.frame2,bg = '#3b3838')
		self.frame22.pack(fill = 'both',expand = 1)
		self.enc = Button(self.frame22,text = 'ENCRYPT',command = self.Encrypt,relief = 'groove',font = ('Russo One',10),activebackground = '#4d4d4d',bg = '#4d4d4d',borderwidth = 0,fg = '#cdcdcd')
		self.enc.pack(side = 'left',expand = 1,fill = 'both',padx = 4,pady = 4)
		self.c = Button(self.frame22,text = 'CLEAR',command = self.Clear,relief = 'groove',font = ('Russo One',10),activebackground = '#4d4d4d',bg = '#4d4d4d',borderwidth = 0,fg = '#cdcdcd')
		self.c.pack(expand = 1,fill = 'both',side = 'left',padx = 4,pady = 4)
		self.dec = Button(self.frame22,text = 'DECRYPT',command = self.Encrypt,relief = 'groove',font = ('Russo One',10),activebackground = '#4d4d4d',bg = '#4d4d4d',borderwidth = 0,fg = '#cdcdcd')
		...
		self.admin_set = Button(self.frame23,text = 'ADMIN SETTINGS',command = self.ad_set,relief = 'groove',font = ('Russo One',10),activebackground = '#4d4d4d',bg = '#4d4d4d',borderwidth = 0,fg = '#cdcdcd')
		self.admin_set.pack(expand = 1,side = 'left',fill = 'both',padx = 4,pady = 4)
		self.sf = Button(self.frame23,text = 'SAVE AS FILE',command = self.saf,relief = 'groove',font = ('Russo One',10),activebackground = '#4d4d4d',bg = '#4d4d4d',borderwidth = 0,fg = '#cdcdcd')
		self.sf.pack(expand = 1,fill = 'both',side = 'left',padx = 4,pady = 4)
		self.op = Button(self.frame23,text = 'OPEN FILE',command = self.open_file_f2,relief = 'groove',font = ('Russo One',10),activebackground = '#4d4d4d',bg = '#4d4d4d',borderwidth = 0,fg = '#cdcdcd')
		self.op.pack(expand = 1,side = 'left',fill = 'both',padx = 4,pady = 4)
		self.log_out = Button(self.frame23,command = self.logout,text = 'LOG-OUT',relief = 'groove',font = ('Russo One',10),activebackground = '#4d4d4d',bg = '#4d4d4d',borderwidth = 0,fg = '#cdcdcd')
	...
	def drop_inside_text_box_emp(self,event):
		self.data = event.data[1:-1]
		if event.data.endswith('.txt}'):
			with open(self.data,'r') as self.file:
				for i in self.file:
					self.editor.insert(END,f'{i}')
		else:
			messagebox.showerror('Error','File Cannot Be Opened!!!')
	...
	def Frame3(self):
		...
		self.editor.dnd_bind('<<Drop>>',self.drop_inside_text_box_emp)
		...
		self.l0 = Label(self.frame_e,text = 'Font',font = ('Russo One',15),bg = '#f9b04a',fg = '#3b3838')
		...
		self.l1 = Label(self.frame_e,text = 'Font Size',font = ('Russo One',15),bg = '#f9b04a',fg = '#3b3838')
		...
		self.l2 = Label(self.frame_e,text = 'Font Style',font = ('Russo One',15),bg = '#f9b04a',fg = '#3b3838')
